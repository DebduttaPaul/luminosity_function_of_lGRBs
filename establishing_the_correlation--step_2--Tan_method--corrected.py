...
print 'size of array		:	', common_Luminosity.size
print 'minimum luminosity	:	', common_Luminosity.min()
...
#~ #	A_min	=	1e-3	;	A_max	=	 10.00	;	A_bin	=	0.50	#2
#~ #	eta_min	=	1e-3	;	eta_max	=	 3.00	;	eta_bin	=	0.10	#2
#~ #	A_min	=	1e-3	;	A_max	=	 10.00	;	A_bin	=	0.50	#3
#~ #	eta_min	=	1e-3	;	eta_max	=	 2.00	;	eta_bin	=	0.10	#3
#~ #	A_min	=	1e-3	;	A_max	=	 7.00	;	A_bin	=	0.50	#4
#~ #	eta_min	=	1e-3	;	eta_max	=	 1.50	;	eta_bin	=	0.10	#4
#~ A_min	=	0.50	;	A_max	=	 4.00	;	A_bin	=	5e-2	#5
#~ eta_min	=	1e-3	;	eta_max	=	 1.00	;	eta_bin	=	5e-2	#5
#~ 
...
#~ 
#~ levels = range( 90, 120, 1 )										#5
#~ plt.contourf( A_array, eta_array, grid_of_discrepancy, levels )		#5
#~ #	plt.contourf( A_array, eta_array, grid_of_discrepancy )				#1,2,3,4
#~ plt.colorbar()
#~ plt.xlabel( r'$ \mathrm{ A } $', fontsize = size_font + 2, labelpad = padding - 3 )
#~ plt.ylabel( r'$ \eta $', fontsize = size_font + 2, labelpad = padding )
#~ plt.savefig( './../plots/Tan_method--corrected/discrepancy_contours.png' )
#~ plt.clf()
#~ plt.close()
#~ 
#~ plt.contourf( A_array, eta_array, grid_of_redchisqrd )
#~ plt.colorbar()
#~ plt.xlabel( r'$ \mathrm{ A } $', fontsize = size_font + 2, labelpad = padding - 3 )
#~ plt.ylabel( r'$ \eta $', fontsize = size_font + 2, labelpad = padding )
#~ plt.savefig( './../plots/Tan_method--corrected/redchisqrd_contours.png' )
#~ plt.clf()
#~ plt.close()
#~ 
...
A___myredshift	=	2.25	;	eta_myredshift	=	0.30					#	Implementing Tan's method without removing the low-Luminosity branch.
...
plt.savefig( './../plots/Tan_method--corrected/distribution--Yonetoku.png' )
plt.clf()
plt.close()
...
plot_distribution( returned[0] )
plt.savefig( './../plots/Tan_method--corrected/distribution--Tan_bestfit.png' )
plt.clf()
plt.close()
...
plot_distribution( returned[0] )
plt.savefig( './../plots/Tan_method--corrected/distribution--Tan_redshift.png' )
plt.clf()
plt.close()
...
plot_distribution( returned[0] )
plt.savefig( './../plots/Tan_method--corrected/distribution--my_bestfit.png' )
plt.clf()
plt.close()

returned	=	discrepancy_and_redchisqrd( A___myredshift , eta_myredshift  )
print 'my redshift :	', returned[2], returned[3], returned[4], returned[5]
plot_distribution( returned[0] )
plt.savefig( './../plots/Tan_method--corrected/distribution--my_redshift.png' )
plt.clf()
plt.close()
...
ax.plot( x_to_fit_in_keV, ( L_norm * A___Tanredshift )  * ( x_to_fit_in_MeV**eta_Tanredshift ), color = 'y'         , label = r'$ \rm{ Tan \, (2013) : new \, method  } $' )
ax.plot( x_to_fit_in_keV, ( L_norm * A___mybestfit   )  * ( x_to_fit_in_MeV**eta_mybestfit   ), color = 'g'         , label = r'$ \rm{ present \, work : best \, fit } $' )
ax.plot( x_to_fit_in_keV, ( L_norm * A___myredshift  )  * ( x_to_fit_in_MeV**eta_myredshift  ), color = 'darkorange', label = r'$ \rm{ present \, work : Tan \, method \, (corrected)  } $' )
plt.legend( numpoints = 1, loc = 'upper left' )
plt.savefig( './../plots/Tan_method--corrected/L_vs_Ep(1+z)--correlations--Tan_method_corrected.png' )
plt.clf()
plt.close()
